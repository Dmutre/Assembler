...
; Needed functions: MessageBox, FloatToStr2, wsprintf, LoadLibrary, GetProcAddress, FreeLibrary, ExitProcess


.data 
  zero_cause_msg_box_title db "Error has appeared", 0
  msg_box_title db "No entry dynamic ( 4 * lg(c) - b/2 + 23)/(d - a + 1)", 0
...
main:
  ...
  valueLoop:
    ...
    mov customLib, eax 
    invoke GetProcAddress, customLib, addr procedure
    mov address, eax
    ...
    lea ecx, Dvalue[esi*8]
    push ecx
    lea ecx, Cvalue[esi*8]
    push ecx
    lea ecx, Bvalue[esi*8]
    push ecx
    lea ecx, Avalue[esi*8]
    push ecx
